...
const AvailabilityCell = ({ rowData, headers }) => {
  ...
  const id = rowData.selectedSku ? rowData.selectedSku.uniqueID : rowData.uniqueID;
...
const useBundle = (props) => {
    ...
    inventoryavailabilityService
      ...
      .then((res) => {
        ...
        get(res, "data.InventoryAvailability", []).forEach(
          ({ inventoryStatus: s, productId: id, availableQuantity: quantity }) =>
  ...
  const addToRL = useCallback(
    async (requisitionListId) => {
      const { i: u, q, e } = _getItems(rows, headers, tableState);
      let n = 0;
      if (!e.errorMessage) {
  ...
  const Add2CartOrReqButton = useCallback(() => {
    ...
    const someInvalid = rows.some((r) => {
      ...
      if (!isProdOrVar(r) || r.selectedSku) {
        ...
        return (!iv.available && q > 0) || (iv.available && parseInt(iv.quantity) < q);
      } else if (q > 0) {
        someWithNoSkus = true;
        return true;
      } else {
        ...
        <StyledGrid container item xs={12} spacing={1} alignItems="center" className="button-combo">
          ...
          {loginStatus && isB2B ? (
            <StyledGrid item xs={12} sm="auto">
              <AddToRequisitionListButton {...{ disabled, addFn: addToRL }} />
            </StyledGrid>
          ) : null}
        </StyledGrid>
        {disabled && (
