...
//hcl packages
import { ContentCarouselWidget, StyledProgressPlaceholder } from "@hcl-commerce-store-sdk/react-component";
      ...
      if (obj.contentFormatName === "Text") {
        ...
        attachmentAsset.forEach((a) => {
          ...
          const attachmentData = (
            <Link to={contentUrl} onClick={(event) => performClick(event, { eSpotDesc: content })}>
...
export const withContentCarouselWidget =
  ...
  ({ widget, page, ...props }) => {
    ...
    return <>{slideList && slideList.length > 0 && <WrapComponent {...{ slideList, controls }}></WrapComponent>}</>;
  };
