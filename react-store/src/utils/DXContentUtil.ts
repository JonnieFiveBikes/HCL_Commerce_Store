...
const DXContentUtil = {
  ...
  getdataMap: (content: any) => {
    dataMap.clear();
    dxData.clear();
    const elements = content?.content?.elements.element;
...
const dxDataHelper = {
  buttonLink: () => {
    if (dataMap.get(CONTENT_TEMPLATE.BUTTON_LINK)) {
      const text: any = dataMap.get(CONTENT_TEMPLATE.BUTTON_LINK);
  ...
  buttonText: () => {
    if (dataMap.get(CONTENT_TEMPLATE.BUTTON_TEXT_DISPLAY)) {
      const text: any = dataMap.get(CONTENT_TEMPLATE.BUTTON_TEXT_DISPLAY);
      dxData.set(CONTENT_TEMPLATE.BUTTON_TEXT_DISPLAY, text.value);
    }
  },
  imageData: () => {
    if (dataMap.get(CONTENT_TEMPLATE.IMAGE)) {
      const imageVal: any = dataMap.get(CONTENT_TEMPLATE.IMAGE);
      let imageValue = imageVal.image?.resourceUri?.value;
      if (!imageValue) {
        const val: any = dataMap.get(CONTENT_TEMPLATE.IMAGE_ALTERNATE_URL);
  ...
  imageUsage: () => {
    if (dataMap.get(CONTENT_TEMPLATE.IMAGE_USAGE)) {
      ...
      const value = dxDataHelper.getOptionSelection(dataMap, CONTENT_TEMPLATE.IMAGE_USAGE);
  ...
  video: () => {
    const elem = dataMap.get(CONTENT_TEMPLATE.VIDEO);
    if (elem) {
      let AltURL: any;
  ...
  text: () => {
    if (dataMap.get(CONTENT_TEMPLATE.TEXT)) {
      const text: any = dataMap.get(CONTENT_TEMPLATE.TEXT);
      dxData.set(CONTENT_TEMPLATE.TEXT, text.value);
    }
  },
  textElementWidth: () => {
    if (dataMap.get(CONTENT_TEMPLATE.TEXT_ELEMENT_WIDTH)) {
      const value = dxDataHelper.getOptionSelection(dataMap, CONTENT_TEMPLATE.TEXT_ELEMENT_WIDTH);
      dxData.set(CONTENT_TEMPLATE.TEXT_ELEMENT_WIDTH, value);
    }
  },
  textAlignment: () => {
    if (dataMap.get(CONTENT_TEMPLATE.TEXT_ALIGNMENT)) {
      const value = dxDataHelper.getOptionSelection(dataMap, CONTENT_TEMPLATE.TEXT_ALIGNMENT);
  ...
  textElementPaddingDesktop: () => {
    if (dataMap.get(CONTENT_TEMPLATE.TEXT_ELEMENT_PADDING_DESKTOP)) {
      const textElement: any = dataMap.get(CONTENT_TEMPLATE.TEXT_ELEMENT_PADDING_DESKTOP);
  ...
  textElementPaddingTablet: () => {
    if (dataMap.get(CONTENT_TEMPLATE.TEXT_ELEMENT_PADDING_TABLET)) {
      const textElement: any = dataMap.get(CONTENT_TEMPLATE.TEXT_ELEMENT_PADDING_TABLET);
  ...
  textElementPaddingMobile: () => {
    if (dataMap.get(CONTENT_TEMPLATE.TEXT_ELEMENT_PADDING_MOBILE)) {
      const textElement: any = dataMap.get(CONTENT_TEMPLATE.TEXT_ELEMENT_PADDING_MOBILE);
  ...
  buttonAppearance: () => {
    if (dataMap.get(CONTENT_TEMPLATE.BUTTON_APPEARANCE)) {
      ...
      const value = dxDataHelper.getOptionSelection(dataMap, CONTENT_TEMPLATE.BUTTON_APPEARANCE);
  ...
  heightDesktop: () => {
    if (dataMap.get(CONTENT_TEMPLATE.HEIGHT_DESKTOP)) {
      const heightData: any = dataMap.get(CONTENT_TEMPLATE.HEIGHT_DESKTOP);
  ...
  heightTablet: () => {
    if (dataMap.get(CONTENT_TEMPLATE.HEIGHT_TABLET)) {
      const heightData: any = dataMap.get(CONTENT_TEMPLATE.HEIGHT_TABLET);
  ...
  heightMobile: () => {
    if (dataMap.get(CONTENT_TEMPLATE.HEIGHT_MOBILE)) {
      const heightData: any = dataMap.get(CONTENT_TEMPLATE.HEIGHT_MOBILE);
  ...
  elementAlignment: () => {
    if (dataMap.get(CONTENT_TEMPLATE.ELEMENT_ALIGNMENT)) {
      ...
      const value = dxDataHelper.getOptionSelection(dataMap, CONTENT_TEMPLATE.ELEMENT_ALIGNMENT);
      if (value === "Start") {
  ...
  borderRadius: () => {
    if (dataMap.get(CONTENT_TEMPLATE.BORDER_RADIUS)) {
      const borderData: any = dataMap.get(CONTENT_TEMPLATE.BORDER_RADIUS);
  ...
  dropShadow: () => {
    if (dataMap.get(CONTENT_TEMPLATE.DROP_SHADOW)) {
      ...
      const value = dxDataHelper.getOptionSelection(dataMap, CONTENT_TEMPLATE.DROP_SHADOW);
  ...
  layoutDirection: () => {
    if (dataMap.get(CONTENT_TEMPLATE.LAYOUT_DIRECTION)) {
      let status: any = "row";
      const value = dxDataHelper.getOptionSelection(dataMap, CONTENT_TEMPLATE.LAYOUT_DIRECTION);
      if (value === "Column") {
        status = "column";
      }
  ...
  videoUsage: () => {
    let status: boolean = false;
    const value = dxDataHelper.getOptionSelection(dataMap, CONTENT_TEMPLATE.USE_AS_BACKGROUND_VIDEO);
  ...
  videoControl: () => {
    if (dataMap.get(CONTENT_TEMPLATE.VIDEO_CONTROLS)) {
      let status: boolean = false;
      const value = dxDataHelper.getOptionSelection(dataMap, CONTENT_TEMPLATE.VIDEO_CONTROLS);
  ...
  videoVolume: () => {
    if (dataMap.get(CONTENT_TEMPLATE.VIDEO_VOLUME)) {
      let status: boolean = false;
      const value = dxDataHelper.getOptionSelection(dataMap, CONTENT_TEMPLATE.VIDEO_VOLUME);
  ...
  videoPoster: () => {
    const elem = dataMap.get(CONTENT_TEMPLATE.VIDEO_POSTER);
    if (elem) {
      let AltURL: any;
  ...
  mlTranslation: () => {
    if (dataMap.get(CONTENT_TEMPLATE.ML_TRANSLATION)) {
      const text: any = dataMap.get(CONTENT_TEMPLATE.ML_TRANSLATION);
