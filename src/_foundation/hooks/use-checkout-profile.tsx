...
export interface CheckoutProfileType
  extends PersonCheckoutProfileCheckoutProfile {
...
export const useCheckoutProfile = () => {
  ...
  const { cProf, setCProf, setActiveStep, updateProfile, setUpdateProfile } =
  ...
  useEffect(
    () => {
      ...
      if (submitted) {
        history.push(ROUTES.CHECKOUT_PROFILES);
      }
    },
    // eslint-disable-next-line react-hooks/exhaustive-deps
    [fromState]
  ...
  const canSubmit = () => {
    ...
    return (
      ...
      (p === "COD" ||
        (p && !invalidCC(cProf.billingData, true) && y && m && !inPast))
    );
  };
  ...
  const submitCheckoutProfile = async (list) => {
    ...
    const shipData = getObjWithPrefix(
      list,
      cProf.shipping_nickName,
      "shipping_"
    );
  ...
  const cancel = () => history.push(ROUTES.CHECKOUT_PROFILES);
