...
const useUtils = () => {
  ...
  const addKeysToCart = ({ keys, t, h, s, f }) => {
...
const MultiDeleteAction = ({ fullTable: tbl, headers: h, ...props }) => {
  ...
  const { deleteKeys, findSelectedKeys, getValueForCell, getCurrentContext } =
    useUtils();
...
const DeleteAction = ({ rowData: r, fullTable: t, headers: h, ...props }) => {
  const { mySite } = useSite();
  const { tableState: s, setTableState: f } = useCustomTable();
  const { getRowKey, deleteKeys, getValueForCell, getCurrentContext } =
    useUtils();
  const cart = useSelector(cartSelector);
  const orderId = get(getCurrentContext(s), "orderId");
  const forUserId = useSelector(forUserIdSelector);
...
const QuantityCell = ({ rowData: r, headers: h }) => {
  const { mySite } = useSite();
  const { setValueForCell, getValueForCell, getCurrentContext } =
    useTableUtils();
  const { tableState: s, setTableState: f } = useCustomTable();
  const dispatch = useDispatch();
  const cart = useSelector(cartSelector);
...
export const useInprogressItems = (props: InprogressItemsHookProps) => {
  ...
  const { setCurrentContextValue, getCurrentContext, getValueForCell } =
    useTableUtils();
  const cart = useSelector(cartSelector);
  const forUserId = useSelector(forUserIdSelector);
  const uId = useSelector(userIdSelector);
    ...
    const columns: TableColumnDef[] = [
              ...
              <StyledGrid container>
                ...
                <StyledGrid item xs={12} sm style={{ wordBreak: "break-word" }}>
                  <StyledTypography variant="body2">
                    {rowData.seo?.href ? (
                      <StyledLink to={rowData.seo.href}>
      ...
      },
      {
        ...
        display: {
          cellStyle,
          template: ({ rowData, ...props }) => (
            <>{dFmt.format(new Date(rowData.createDate))}</>
          ),
        },
        sortable: {
      ...
      },
      {
        ...
        sortable: {
          fn: ({ rowData: r, tableState: s }) => {
            ...
            const rc =
              getValueForCell("quantity", r, h, s) ?? parseInt(r.quantity);
            return rc;
          },
          numeric: true,
        },
      },
      {
        ...
        sortable: {
          fn: ({ rowData: { xitem_firstName: f, xitem_lastName: l } }) =>
            `${f}${l}`,
        },
      },
      {
