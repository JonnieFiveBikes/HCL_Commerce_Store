...
const ThumbnailCell = ({ rowData, ...props }) => {
  ...
  return (
    <>
      {rowData.seo && rowData.seo.href ? (
        <Link to={rowData.seo?.href}>{A}</Link>
      ) : (
        <>{A}</>
      )}
    </>
...
export const useMultipleShipmentTable = (props: any) => {
  ...
  const data = useMemo(
    () =>
      (dataProps ?? []).map((oi) => ({
        ...
        __validAddress__: !!(usableAddresses ?? []).find(
          ({ addressId: a }) => a === oi.addressId
        ),
      })),
    [
      dataProps,
  ...
  const MultiAssignAction = ({ fullTable: tbl, headers: h, ...props }) => {
    ...
    return (
      <StyledButton
        data-testid="group-select-shipping-address"
        color="primary"
        onClick={() => multiSelect(findSelectedKeys(s))}
