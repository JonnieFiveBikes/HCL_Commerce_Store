...
const useProductDetails = (page: Page) => {
  ...
  let cancels: Canceler[] = useMemo<Canceler[]>(() => [], []);
  ...
  const makeSkusAsAttrs = useCallback(
    (skus: any[]) => skus.map((s) => attrsAsMap(s.attributes)),
    [attrsAsMap]
  );
  ...
  const initializeProduct = (productInfo: any, attr?: any) => {
    let newSelection = { ...CURRENT_SELECTION_INIT };
    if (productInfo) {
      ...
      if (productInfo.items?.length > 0) {
        ...
        const firstSkuWithAttrs = items.find(({ attributes: a }) => a);
        newSelection.sku = [];
              ...
              if (att.identifier === "Color") {
                for (const v of att.values || []) {
                  if (Array.isArray(newSelection.selectedAttributes["Color"])) {
                    const selectedColorAttributes =
                      newSelection.selectedAttributes["Color"];
                    if (
                      selectedColorAttributes.length !== 0 &&
                      v.identifier === selectedColorAttributes[0]
                    ) {
                      setSkuColor(v.value);
                    }
                  } else {
                    if (
                      v.identifier === newSelection.selectedAttributes["Color"]
                    ) {
                      setSkuColor(v.value);
                    }
                  }
  ...
  const search = useCallback((partialAttr, fullAttr, key, list) => {
    ...
    const best =
      exact === -1 ? list.findIndex((s) => s[key] === partialAttr[key]) : -1;
    return { exact, best };
  }, []);
  ...
  const formattedPriceDisplayOffer = (
    <FormattedPriceDisplay min={displayOfferPrice} />
  );
  const formattedPriceDisplayList = (
