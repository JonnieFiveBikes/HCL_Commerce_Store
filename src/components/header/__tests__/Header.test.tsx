...
describe("Mobile : Header is displayed in mobile version ", () => {
  test("Mobile : Header is displayed in mobile version ", () => {
    ...
    const SearchBarMobileElement = screen.getByTestId(
      "search-bar-mobile-element"
    );
    expect(SearchBarMobileElement).toBeInTheDocument();

    expect(screen.queryByTestId("menubar-menuitem-button")).toBeNull();
    expect(
      screen.queryByText("AllCategoriesExpandedMenu.AllCategoriesLabel")
    ).toBeNull();
    expect(
      screen.queryByTestId("search-bar-desktop-largetablet-element")
    ).toBeNull();
  });
});
...
describe("Desktop : Header is displayed in desktop version", () => {
  test("Desktop : Header is displayed in desktop version", () => {
    ...
    const MenubarMenuItemElement = screen.getByTestId(
      "menubar-menuitem-button"
    );
    expect(MenubarMenuItemElement).toBeInTheDocument();

    const AllCategoriesExpandedMenuElement = screen.getByText(
      "AllCategoriesExpandedMenu.AllCategoriesLabel"
    );
    expect(AllCategoriesExpandedMenuElement).toBeInTheDocument();

    const SearchBarDesktopLargeTabletElement = screen.getByTestId(
      "search-bar-desktop-largetablet-element"
    );
