...
const CheckoutProfileBilling = (props: any) => {
  ...
  const { activeStep, cProf, setCProf, updateProfile } =
    useCheckoutProfileContext();
  ...
  useEffect(() => {
    ...
    const billingList = contactList.filter((address) =>
      address.addressType.includes("Billing")
    );
    ...
    // set use-same state
    if (
      m[cProf.billing_nickName] &&
      cProf.shipping_nickName === cProf.billing_nickName
    ) {
    ...
    <StyledPaper className="vertical-padding-4">
        ...
        <StyledGrid container spacing={2}>
                ...
                components={[
                  <StyledLink
                    to={{
                      ...
                      state: { profile: cProf, activeStep },
                    }}
                  />,
                ]}
              />
              ...
              <StyledFormControlLabel
                control={
                  <StyledCheckbox
                    ...
                    color="primary"
                    onChange={(e) =>
                      setUseShippingAddressForBilling(e.target.checked)
                    }
                  />
                }
          ...
          <StyledGrid item xs={12}>
            {editAddress ? (
                ...
                <StyledGrid
                  ...
                  <StyledGrid item>
                    <StyledButton
                      color="primary"
                      disabled={
                        !addressUtil.validateAddressForm(
                          editAddressFormData,
                          true
                        )
                      }
              ...
              <AddressContext.Provider
                value={{ toggleEditAddress, setEditAddressFormData }}>
                <AddressList
                  cid="addresses"
                  {...{
                    addressList: useShippingAddressForBilling
                      ? addressList.filter(
                          ({ nickName: n }) => n === cProf.shipping_nickName
                        )
                      : addressList,
                    selectedNickName: useShippingAddressForBilling
                      ? cProf.shipping_nickName
                      : cProf.billing_nickName,
                    setSelectedAddressId,
                  }}
                />
              </AddressContext.Provider>
          ...
          <StyledGrid item xs={12}>
            <StyledFormControl variant="outlined">
              ...
              <StyledSelect
                data-testid="cprof-billing-method-select"
                native
                fullWidth
                value={
                  payMethods?.find(
                    ({ policyName: p }) =>
                      p === cProf?.billingData?.payment_method?.value
                  )?.policyName
                }
            ...
            <>
              <StyledGrid item xs={12}>
                <StyledTextField
                  ...
                  helperText={
                    invalidCC(cProf.billingData, dirty)
      ...
      <StyledGrid
        ...
        <StyledButton
          ...
          color="primary"
          disabled={
            !(nnMap[cProf.billing_nickName] || useShippingAddressForBilling) ||
            !canSubmit()
          }
          onClick={() => submitCheckoutProfile(fromState.contactList)}
          className="button bottom-margin-1">
          {updateProfile
            ? t("CheckoutProfile.UpdateProfile")
            : t("CheckoutProfile.CreateProfile")}
        </StyledButton>
      </StyledGrid>
    </StyledPaper>
  );
