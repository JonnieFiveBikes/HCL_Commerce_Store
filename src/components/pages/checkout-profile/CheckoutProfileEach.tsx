    ...
    <>
      {lines.map((o, i) => {
        if (o.m && Array.isArray(o.v)) {
          return o.v.map((l, j) => (
            <StyledTypography key={`${i}_${j}`}>{l}</StyledTypography>
          ));
        } else {
          const disp = Array.isArray(o.v) ? o.v.filter(Boolean).join(o.j) : o.v;
          return disp ? (
            <StyledTypography key={i}>{disp}</StyledTypography>
          ) : null;
        }
      })}
    </>
...
export const CheckoutProfileEach: React.FC<CheckoutProfileEachProps> = ({
  profile: p,
}) => {
  ...
  const EditCheckoutProfile = () =>
    history.push(ROUTES.CHECKOUT_PROFILE_EDIT, { profile: p });
            ...
            <StyledGrid
              ...
              <StyledGrid item xs={6} sm={6} md={3}>
                <StyledTypography variant="overline">
                  {t("CheckoutProfile.ShippingAddr")}
                </StyledTypography>
              ...
              <StyledGrid item xs={6} sm={6} md={3}>
                <StyledTypography variant="overline">
                  {t("CheckoutProfile.ShippingMethodDisp")}
                </StyledTypography>
                <StyledTypography>
                  {modes[p.shipping_modeId as string]?.shipModeDescription || (
                    <span>&nbsp;</span>
                  )}
                </StyledTypography>
              </StyledGrid>
              ...
              <StyledGrid item xs={6} sm={6} md={3}>
                <StyledTypography variant="overline">
                  {t("CheckoutProfile.BillingMethod")}
                </StyledTypography>
                <StyledTypography>
                  {t(
                    `CheckoutProfile.payMethods.${p.billingData.payment_method.value}`
                  )}
                </StyledTypography>

                {p.billingData.payment_method.value !== "COD" ? (
                  <StyledTypography>
                    {p.billingData.account.value}
                  </StyledTypography>
                ) : null}
              </StyledGrid>
            </StyledGrid>
