...
const useShipping = () => {
  ...
  const [useMultipleShipment, setUseMultipleShipment] =
    useState<boolean>(false);
  const [selectShipment, setSelectShipment] = useState<boolean>(false);
  const [skipMultipleShipment, setSkipMultipleShipment] =
    useState<boolean>(false);
  ...
  async function confirmShipInfo() {
    ...
    selectedItems.forEach((item) => {
      let orderItem = orderItems.filter(
        (orderItem) => orderItem.orderItemId === item.orderItemId
      )[0];
      let orderItemId = orderItem.orderItemId;
      let shipModeId = selectedShipModeIds[0];
      let addressId = selectedShipAddressIds[0];
      let orderItemJson = {
  ...
  const clickCheckbox = (evt: any) => {
    ...
    if (evt.target.value === "-1") {
      ...
      if (evt.target.checked) {
        setItemsMap(
          Object.assign(iMap, itemsMap, { [evt.target.value]: true })
        );
        ...
        setItemsMap(
          Object.assign(iMap, itemsMap, { [evt.target.value]: false })
        );
        itemsList = tempItemsList;
  ...
  function handleSelectShipmentChangeForCheckboxes() {
    ...
    if (selectAllCheckboxes) {
      ...
      setSelectedItems(
        tempItemsList.map((itemId) =>
          orderItems.find((item) => item.orderItemId === itemId)
        )
      );
    }
  ...
  useEffect(() => {
    ...
    const allCheck: boolean = orderItems.every(
      (item) => itemsMap[item.orderItemId] === true
    );
