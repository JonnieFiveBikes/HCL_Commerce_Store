...
const PaymentInfoCard: React.FC<PaymentInfoCardProps> = (props: any) => {
  ...
  const addressId = paymentInfo.addressId
    ? paymentInfo.addressId
    : paymentInfo.billing_address_id;
  ...
  const addressData = props.addressData
    ...
    : getAddress();
  const maxPiAmount = getMaximumPiAmount
  ...
  function buildAddressData(address: any) {
    ...
    if (
      ...
      finalAddressData.firstName !== EMPTY_STRING
    ) {
      fullNameString = finalAddressData.firstName;
    }
    if (
      finalAddressData.lastName !== undefined &&
      finalAddressData.lastName !== EMPTY_STRING
    ) {
  ...
  function getPaymentMethodDisplayName() {
    ...
    if (paymentInfo && payMethods?.length > 0) {
      const payMethod = payMethods.find(
        (pm) => pm.paymentMethodName === paymentInfo.payMethodId
      );
      if (payMethod) {
  ...
  function getCreditCardDetails() {
    ...
    if (paymentInfo && paymentInfo.creditCardFormData) {
      ...
      account = paymentInfo.protocolData.find(
        (pd) => pd.name === ACCOUNT_CC
      )?.value;
      expire_month = paymentInfo.protocolData.find(
        (pd) => pd.name === EXPIRE_MONTH
      )?.value;
      expire_year = paymentInfo.protocolData.find(
      ...
      <StyledGrid item xs={12} sm={6}>
        ...
        {readOnly ? (
          <StyledTypography variant="body2" gutterBottom>
            <FormattedPriceDisplay
              min={parseFloat(paymentInfo.piAmount)}
              currency={paymentInfo.piCurrency}
            />
          </StyledTypography>
        ) : (
          <StyledNumberInput
        ...
        {account && account !== EMPTY_STRING && (
          <StyledTypography variant="body2">
            {storeUtil.maskCardNumber(account)}
          </StyledTypography>
        )}
