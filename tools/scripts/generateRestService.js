...
//verify cms args
if (
  cmdArgs.spec === "undefined" ||
  cmdArgs.name === "undefined" ||
  cmdArgs.resourceDomain === "undefined"
) {
...
function generateRestClass(opts) {
  let classTemplate = fs.readFileSync(
    "tools/scripts/codeGen/templates/swagger/clazz.mustache",
    "utf-8"
  );
  let methodTemplate = fs.readFileSync(
    "tools/scripts/codeGen/templates/swagger/method.mustache",
    "utf-8"
  );
  let requestTemplate = fs.readFileSync(
    "tools/scripts/codeGen/templates/swagger/request.mustache",
    "utf-8"
  );
  ...
  if (opts.tag && opts.tag !== "undefined") {
    ...
    if (filteredTags.length === 0) {
      console.error(
        "Error: The specified tag " +
          opts.tag +
          "  is not present in Swagger spec " +
          opts.spec +
          "."
      );
      return;
  ...
  opts.camelCaseResourceDomain = opts.resourceDomain
    .substr(0, 1)
    .toLowerCase()
    .concat(opts.resourceDomain.substr(1));
  opts.camelCaseClassName = opts.name
    .substr(0, 1)
    .toLowerCase()
    .concat(opts.name.substr(1));
  //generate
